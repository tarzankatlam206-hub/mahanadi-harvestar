...
useEffect(()=>{let p=0;const iv=setInterval(()=>{p=p+2;if(p>=100){p=100;clearInterval(iv);setTimeout(()=>setSplash(false),400);}setProgress(p);},30);return ()=>clearInterval(iv);},[]);
...
useEffect(()=>{const onBackPress=()=>{if(show){setShow(false);return true;}if(view!=="home"){setView("home");return true;}if(isLogin&&view==="home"){AsyncStorage.setItem("isLogin","no");setIsLogin(false);return true;}return false;};const sub=BackHandler.addEventListener("hardwareBackPress",onBackPress);return ()=>sub.remove();},[view,show,isLogin]);
...
const save=()=>{const id=editId||Date.now().toString();const data=Object.assign({},form,{id});
...
const delItem=(it)=>{if(type==="members")setMembers(p=>p.filter(x=>x.id!==it.id));if(type==="kisan")setKisans(p=>p.filter(x=>x.id!==it.id));if(type==="agent")setAgents(p=>p.filter(x=>x.id!==it.id));if(type==="operator")setOperators(p=>p.filter(x=>x.id!==it.id));if(type==="helper")setHelpers(p=>p.filter(x=>x.id!==it.id));if(type==="dealer")setDealers(p=>p.filter(x=>x.id!==it.id));if(type==="parts")setParts(p=>p.filter(x=>x.id!==it.id));if(type==="notice")setNotices(p=>p.filter(x=>x.id!==it.id));};

if(splash){
const sc=0.5+(progress/200);
const barW=progress*2.6;
return(
<View style={s.splashNew}>
<View style={{transform:[{scale:sc}]}}>
<Image source={require("./assets/splash.png")} style={s.splashImg} resizeMode="contain" />
</View>
<Text style={s.splashTitle}>महानदी हार्वेस्टर</Text>
<View style={s.barBg}><View style={[s.barFill,{width:barW}]} /></View>
<Text style={s.barTxt}>लोडिंग {progress}%</Text>
</View>
);
}
...
<Text style={s.welcomeTitle}>महानदी हार्वेस्टर मालिक कल्याण संघ जिला कांकेर (छत्तीसगढ़) में आपका स्वागत है</Text>
<Text style={s.welcomeSub}>हार्वेस्टर मालिकों का विश्वसनीय सहकारी मंच</Text>
</View>
...
<Text style={s.addressText}>पता- लखनपुरी, मेन रोड़, N.H.30, जिला सहकारी बैंक के सामने, ब्लॉक-चारामा, जिला-कांकेर</Text>
...
{view!=="home" && view!=="logout" && <View style={{flex:1}}><View style={s.sub}><TouchableOpacity onPress={()=>setView("home")}><Text>वापस</Text></TouchableOpacity><Text>{MENU.find(m=>m.key===type)?.title} ({getList().length})</Text><Text></Text></View><View style={s.search}><Text>🔍</Text><TextInput style={{flex:1,padding:8}} value={search} onChangeText={setSearch} placeholder="सर्च करें" /></View><ScrollView>{getList().map(it=><View key={it.id} style={s.card}><Text style={{fontWeight:"bold"}}>{it.name||it.vishay}</Text><Text>{it.mobile||""} {it.pata||""}</Text>{it.mobile?(<View style={{flexDirection:"row",marginTop:10,flexWrap:"wrap"}}><TouchableOpacity style={[s.sm,{backgroundColor:"#4CAF50"}]} onPress={()=>Linking.openURL("tel:"+it.mobile)}><Text style={s.smT}>कॉल</Text></TouchableOpacity><TouchableOpacity style={[s.sm,{backgroundColor:"#128C7E"}]} onPress={()=>Linking.openURL("https://wa.me/91"+it.mobile.toString().replace(/\D/g,"").slice(-10))}><Text style={s.smT}>व्हाट्सएप</Text></TouchableOpacity><TouchableOpacity style={[s.sm,{backgroundColor:"#2196F3"}]} onPress={()=>Linking.openURL("sms:"+it.mobile)}><Text style={s.smT}>मैसेज</Text></TouchableOpacity></View>):null}<View style={{flexDirection:"row",marginTop:8,flexWrap:"wrap"}}><TouchableOpacity style={[s.sm,{backgroundColor:"#FF9800"}]} onPress={()=>openForm(type,it)}><Text style={s.smT}>एडिट करें</Text></TouchableOpacity><TouchableOpacity style={[s.sm,{backgroundColor:"#D32F2F"}]} onPress={()=>delItem(it)}><Text style={s.smT}>डिलीट</Text></TouchableOpacity></View></View>)}</ScrollView><TouchableOpacity style={s.fab} onPress={()=>openForm(type,null)}><Text style={s.fabT}>+</Text></TouchableOpacity></View>}
{view==="logout" && <ScrollView contentContainerStyle={{flexGrow:1,justifyContent:"center",alignItems:"center",padding:15}}><View style={[s.card,{width:"95%",alignItems:"center",padding:20}]}><TouchableOpacity style={{backgroundColor:"#212121",width:"100%",marginTop:10,paddingVertical:22,borderRadius:12,alignItems:"center"}} onPress={doLogout}><Text style={{color:"#ffffff",fontWeight:"900",fontSize:20}}>हाँ, लॉग आउट करें</Text></TouchableOpacity><TouchableOpacity style={{backgroundColor:"#2E7D32",width:"100%",marginTop:20,paddingVertical:22,borderRadius:12,alignItems:"center"}} onPress={()=>setView("home")}><Text style={{color:"#ffffff",fontWeight:"900",fontSize:20}}>नहीं, वापस जाएं</Text></TouchableOpacity></View></ScrollView>}
<Modal visible={show} animationType="slide"><View style={s.modal}><ScrollView style={{padding:12}} contentContainerStyle={{paddingBottom:120}}><Text style={{fontWeight:"bold",textAlign:"center",fontSize:16}}>{MENU.find(m=>m.key===type)?.title} फॉर्म</Text>{Object.keys(form).filter(k=>k!=="id").map(k=><View key={k} style={{marginTop:8}}><Text style={{fontSize:12,fontWeight:"bold"}}>{HINDI[type]?.[k]||k}</Text><TextInput style={s.inp} value={form[k]} onChangeText={t=>setForm({...form,[k]:t})} /></View>)}</ScrollView><View style={s.modalBottom}><TouchableOpacity style={[s.mBtn,{backgroundColor:"#888888"}]} onPress={()=>setShow(false)}><Text style={s.mBtnT}>वापस</Text></TouchableOpacity><TouchableOpacity style={[s.mBtn,{backgroundColor:"#008000"}]} onPress={save}><Text style={s.mBtnT}>सुरक्षित करें</Text></TouchableOpacity></View></View></Modal>
</SafeAreaView>
);
}
...
sub:{flexDirection:"row",justifyContent:"space-between",padding:12,backgroundColor:"#ffffff"},
search:{flexDirection:"row",backgroundColor:"#ffffff",margin:8,paddingHorizontal:10,borderRadius:8,alignItems:"center",borderWidth:1,borderColor:"#FF9800"},
card:{backgroundColor:"#ffffff",margin:8,padding:12,borderRadius:8},
sm:{paddingHorizontal:14,paddingVertical:8,borderRadius:8,marginRight:8,marginBottom:6},
smT:{color:"#ffffff",fontSize:13,fontWeight:"bold"},
...
fabT:{color:"#ffffff",fontSize:28},
inp:{backgroundColor:"#ffffff",borderWidth:1,borderColor:"#cccccc",borderRadius:6,padding:8,marginTop:4},
mBtn:{flex:1,padding:12,borderRadius:8,alignItems:"center",marginRight:6},
mBtnT:{color:"#ffffff",fontWeight:"bold"},
modal:{flex:1,backgroundColor:"#EEF2F7",paddingTop:30},
modalBottom:{flexDirection:"row",padding:12,paddingBottom:30,backgroundColor:"#ffffff",borderTopWidth:1,borderColor:"#dddddd"},
splashNew:{flex:1,backgroundColor:"#000000",justifyContent:"center",alignItems:"center",padding:20},
splashImg:{width:260,height:190},
splashTitle:{fontWeight:"900",fontSize:22,color:"#FFEB3B",marginTop:10,textAlign:"center"},
barBg:{width:260,height:10,backgroundColor:"#ffffff33",borderRadius:10,marginTop:16,overflow:"hidden"},
barFill:{height:10,backgroundColor:"#FFEB3B",borderRadius:10},
barTxt:{color:"#ffffff",fontWeight:"700",marginTop:8,fontSize:14},
...
welcomeSub:{fontWeight:"700",fontSize:12,color:"#0D47A1",textAlign:"center",marginTop:8,backgroundColor:"#FFF9C4",paddingHorizontal:10,paddingVertical:6,borderRadius:8},
loginBox:{width:"90%",backgroundColor:"#ffffff",padding:25,borderRadius:15,alignItems:"center",borderWidth:2,borderColor:"#FF9800"},
...
loginBtnT:{color:"#ffffff",fontWeight:"bold",fontSize:16},
addressBox:{width:"92%",marginTop:15,marginBottom:30,backgroundColor:"#ffffff",borderRadius:12,padding:12,alignItems:"center",borderWidth:1,borderColor:"#FFB300"},
addressTitle:{fontWeight:"900",fontSize:14,color:"#B71C1C",marginBottom:6},
addressText:{fontSize:12,color:"#333333",textAlign:"center"},
phoneText:{fontSize:13,color:"#000000",textAlign:"center",fontWeight:"900",marginTop:6},
emailText:{fontSize:11,color:"#333333",textAlign:"center",marginTop:4}
});
